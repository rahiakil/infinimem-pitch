...
// Replace all placeholder divs with actual Mermaid diagrams
const placeholderRegex = /<div class="diagram-placeholder"[^>]*>[\s\S]*?<\/div>/g;

// Counter to track which diagram we're on
let diagramCount = 0;

// Define the three main diagrams
const diagrams = [
...
graph TD
    subgraph "Infinimem Operating System"
        ...
        B --> C[Memory Agent<br/>🤖 Intelligent Context Manager]
        ...
        H -.->|Cost Optimization| H1[Token Economics<br/>Cost per token ↓ 214x]
        I -.->|Context Expansion| I1[Larger Windows<br/>10M → 50M+ tokens possible]
...
content = content.replace(placeholderRegex, (match) => {
    if (diagramCount < diagrams.length) {
        return diagrams[diagramCount++];
    }
    // For any additional placeholders, just return a simple placeholder
    return `<div class="diagram-placeholder" style="padding: 20px; text-align: center; background: #f5f5f5; border-radius: 8px;">
        <p>📊 Additional Diagram Placeholder</p>
    </div>`;
});
...
console.log(`Successfully restored ${diagramCount} Mermaid diagrams in main index.html`);